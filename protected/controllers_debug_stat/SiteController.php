...
class SiteController extends Controller
{
	...
	public function actions()
	{
		return array(
			...
			'omriUpload' => array(
				'class' => 'application.controllers.site.FileUploadAction',
				'returnUrl' => array('site/analyzeOmri'),
				'report' => true,
				'serverName' => Yii::app() -> basePath . '/../files/omri.csv',
				'formFileName' => 'omriUpload',
				'validate' => function ($file) {
					//print_r($file);
					return true;
					//return $file
	...
	public function actionanalyzeOmri () {
		$reader = new CsvReader(Yii::app() -> basePath.'/../files/omri.csv');
		if ($reader -> file) {
			$reader -> separator = ';';
			...
			while($line = $reader -> line()) {
				...
				//Если не сергей, то нафиг.
				if ($line[0] != 'Сергей') {
					echo "not_serg".$line[0]."<br/>";
					continue;
				} else {
					if ($line[4] != 'подтвержден') {
						echo "not_ver<br/>";
						continue;
					} else {
						$obj = new Omri();
						...
						$obj -> line = $line[2];
						$obj -> status = $line[4];
						$obj -> date = new CDbExpression('FROM_UNIXTIME('.strtotime($line[5]).')');
						$obj -> tel = $line[6];
						$obj -> report = $line[7];
						$obj -> comment = $line[8];
						$obj -> save();
	...
	public function actionPss(){
		echo CPasswordHelper::hashPassword('shubinsa7shubinsa');
	}
